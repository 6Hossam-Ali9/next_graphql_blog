      ...
      <div className="px-4 lg:px-0">
        <div className="flex mb-8 w-full">
          <div className="hidden md:flex items-center justify-center lg:mb-0 lg:w-auto mr-8 ">
            ...
            />
            <p className="text-gray-700 ml-2 font-medium text-lg">
              {post.author.name}
            </p>
          </div>
        ...
        {post.content.raw.children.map((data: any, index: any) => {
          const children = data.children.map((item: any, itemindex: any) =>
            getContentFragment(itemindex, item.text, item)
          );

          return getContentFragment(index, children, data, data.type);
        })}
      </div>
    </div>
  );
